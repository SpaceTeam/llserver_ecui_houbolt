...
sensor_buffer
adc16_channel::command_mapper(
	can_id const id,
	can_message const message
) {
	...
	case command::set_variable_response: {
		...
		sensor_buffer.first[0] = sensor{ .value=value, .node_id=id.node_id, .channel_id=adc16_channel::id, .variable_id=payload.variable_id };
