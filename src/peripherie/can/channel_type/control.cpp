#include "peripherie/can/channel_type/control.h"

#include "utility/Logger.h"

namespace peripherie::can::channel_type {
	sensor_buffer
	control::command_mapper(
		can::id const id,
		...
		case command::status_response:
			log<WARNING>("can command mapper", "CONTROL_STATUS_RESPONSE: not implemented");
			break;
		...
		default:
			log<ERROR>("can command mapper", "control specific command with command id not supported: " + std::to_string(message.command_id));
		}
