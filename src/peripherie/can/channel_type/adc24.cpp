#include "peripherie/can/channel_type/adc24.h"

#include "utility/Logger.h"

namespace peripherie::can::channel_type {
	sensor_buffer
	adc24::command_mapper(
		can::id const id,
		...
		case command::status_response:
			log<WARNING>("can command mapper", "ADC24_STATUS_RESPONSE: not implemented");
			break;
		...
		default:
			log<ERROR>("can command mapper", "adc24 specific command with command id not supported: " + std::to_string(message.command_id));
		}
	...
	adc24::sensor_mapper(
		...
		value |= message.data[offset + 2] << 16;

		if ((0x700 & value) == 0x700) {
			value |= 0xff << 24;
		}
