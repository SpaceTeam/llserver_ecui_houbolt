...
// SMELL: this is a not needed static global variable (make it local)
...
static void set_latency_target(void) {
    // IMPROVE: uninitialized struct type
    struct stat s;
    int ret;
    // IMPROVE: this is not multi-platform but necessary for this program
    // SMELL: this is C code embedded in C++
    if (stat("/dev/cpu_dma_latency", &s) == 0) {
        // SMELL: open is only used in old C and not C++
        latency_target_fd = open("/dev/cpu_dma_latency", O_RDWR);
        if (latency_target_fd == -1)
            return;
        // BUG: write returns size_t ret is int -> implicit narrowing!
        ret = write(latency_target_fd, &latency_target_value, 4);
        if (ret == 0) {
            // SMELL: not C++
            printf("# error setting cpu_dma_latency to %d!: %s\n", latency_target_value, strerror(errno));
            // BUG: file gets only closed when write successful
            close(latency_target_fd);
            return;
        }
        // SMELL: not error Handling or
        // IMPROVE: notification if unsuccessful (write, open)
        // SMELL: not c++
...
int main(int argc, char const *argv[]) {
    // SMELL: System opens a new shell. This is not necessary. Remove line!
    system("clear");
    // IMPROVE: uninitialized struct type
    ...
    // SMELL: inconsistent Naming and redundant improve by removing initialization
    std::string configPath = "";
    // IMPROVE: wtf? why config-file for config-file-path?
    if (argc > 1) {
        // SMELL: cfgPath variable is not needed in this case
        std::string cfgPath(argv[1]);
        configPath = cfgPath;
        // IMPROVE: redundant information this is defined behavior
        ...
        // IMPROVEMENT: redundant information this is defined behavior
